...
// La API Key nunca llega al navegador salvo que el usuario la pase por
...
// e intención, clave para el asistente de voz en planta. Si se satura,
// baja en cascada: 3.5-flash -> 2.5-flash -> 2.5-flash-lite.
...
3. Respuestas Breves: En modo conversacional por voz, responde siempre de forma breve y directa, 2 o 3 frases como máximo salvo que Juanfran pida más detalle explícitamente. Nada de rodeos ni introducciones largas.
...
async function handleGemini(request: Request, env: Env): Promise<Response> {
  ...
  // Intenta primero con el modelo principal; si Google devuelve 503 por
  // sobrecarga, reintenta automáticamente con el modelo de respaldo antes
  // de devolver error al usuario.
  async function runWithModel(modelName: string) {
    const chat = ai.chats.create({ model: modelName, history: chatHistory });
    if (stream) {
  ...
  // Recorre la cadena de modelos: en cada uno, si falla por sobrecarga,
  // espera 2s y lo reintenta una vez antes de bajar al siguiente modelo.
  ...
  for (const modelName of MODEL_CHAIN) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        lastError = err;
        if (!isOverloadError(err)) break; // fallo real (no de saturación): no reintentar en este modelo
        if (attempt === 0) await sleep(2000);
      }
    }
  }
  return jsonResponse({ error: lastError?.message ?? 'Gemini request failed (todos los modelos fallaron)' }, 502);
}
